...
function FieldsList() {
  ...
  const [modifyFieldPopup, setModifyFieldPopup] = useState({
    visible: false,
    type: 0,
    um: 0,
    logic_state: 0,
  ...
  const updateField = (data) => {
    ...
    var fieldToUpdateIndex = upsertType.fields.findIndex(
      (i) => i.QRef === modifyFieldPopup.QRef,
    );
    ...
    fieldToUpdate.fixed_id = data.fixed_id;
    ...
    var actualQuery = upsertType.updateQuery.find(
      (i) => i.QRef === modifyFieldPopup.QRef,
    );
    var newQuery;
    if (actualQuery === undefined) {
      ...
      actualQuery = upsertType.insertQuery.find(
        (i) => i.QRef === modifyFieldPopup.QRef,
      );
      if (actualQuery === undefined) {
        ...
        setUpsertType(
          (prevState) => ({
            ...
            updateQuery: [
              ...
              {
                query: `UPDATE "Field" SET name='${data.name}', type=${data.type}, um=${data.um !== 0 ? data.um : 'NULL'}, logic_state=${data.logic_state !== 0 ? data.logic_state : 'NULL'}, comment=${data.comment === null ? 'NULL' : data.comment === '' ? "''" : `'${data.comment.replace(/'/g, "''")}', fixed_id=${data.fixed_id}`} WHERE name = '${modifyFieldPopup.name}' AND parent_type = typeId;`,
                QRef: fieldToUpdate.QRef,
              },
            ],
            fields: fields,
          }),
          setModifyFieldPopup((prevState) => ({
            ...prevState,
            visible: false,
          })),
        );
        ...
        newQuery[newQuery.findIndex((i) => i.QRef === fieldToUpdate.QRef)] = {
          query: `INSERT INTO "Field" (id, name, type, um, logic_state, comment, fixed_id, parent_type) VALUES (DEFAULT, '${fieldToUpdate.name}', ${fieldToUpdate.type}, ${fieldToUpdate.um !== 0 ? fieldToUpdate.um : 'NULL'}, ${fieldToUpdate.logic_state !== 0 ? fieldToUpdate.logic_state : 'NULL'}, ${fieldToUpdate.comment === null ? 'NULL' : fieldToUpdate.comment === '' ? "''" : `'${fieldToUpdate.comment.replace(/'/g, "''")}'`}, ${fieldToUpdate.fixed_id}, typeId);`,
          QRef: fieldToUpdate.QRef,
        };
        setUpsertType(
          (prevState) => ({
      ...
      newQuery[newQuery.findIndex((i) => i.QRef === fieldToUpdate.QRef)] = {
        query: `UPDATE "Field" SET name='${fieldToUpdate.name}', type=${fieldToUpdate.type}, um=${fieldToUpdate.um}, logic_state=${fieldToUpdate.logic_state}, comment='${fieldToUpdate.comment}', fixed_id=${fieldToUpdate.fixed_id} WHERE name = '${modifyFieldPopup.name}' AND parent_type = typeId;`,
        QRef: fieldToUpdate.QRef,
      };
      setUpsertType(
        (prevState) => ({
  ...
  const deleteField = () => {
    ...
    var deleteQuery =
      upsertType.insertQuery.find((i) => i.QRef !== deletePopup.QRef) ===
      undefined
        ? [
            ...upsertType.deleteQuery,
            {
              query: `DELETE FROM "Field" WHERE name = '${deletePopup.name}' AND parent_type = typeId;`,
              QRef: deletePopup.QRef,
            },
          ]
        : upsertType.deleteQuery;
    ...
    setUpsertType((prevState) => ({
      ...prevState,
      insertQuery: upsertType.insertQuery.filter(
        (i) => i.QRef !== deletePopup.QRef,
      ),
      updateQuery: upsertType.updateQuery.filter(
        (i) => i.QRef !== deletePopup.QRef,
      ),
  ...
  return (
    <>
        ...
        <Table fullWidth className={tableStyles.table}>
          ...
          <TableBody>
            {upsertType.fields.map((item) => {
              ...
              var logic_stateItem = ctx.logicStates.find(
                (i) => i.id === item.logic_state,
              );
              //console.log(upsertType, umItem, logic_stateItem, item)
              return (
                <TableRow key={item.QRef}>
                  ...
                  <TableCell className={tableStyles.cell}>
                    {typeItem !== undefined ? typeItem.name : item.type}
                  </TableCell>
                  <TableCell className={tableStyles.cell}>
                    {item.um !== undefined &&
                      item.um !== 0 &&
                  ...
                  <TableCell className={tableStyles.cell}>
                    {item.logic_state !== undefined &&
                      item.logic_state !== 0 &&
                      item.logic_state !== null &&
                      logic_stateItem.name}
                  </TableCell>
                  <TableCell className={tableStyles.cell}>
      ...
      <ModifyFieldPopup
        ...
        fixed_id={modifyFieldPopup.fixed_id}
