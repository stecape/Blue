...
function VarsList() {
  ...
  const [modifyVarPopup, setModifyVarPopup] = useState({
    visible: false,
    type: 0,
    um: 0,
    logic_state: 0,
  ...
  const updateVar = (data) => {
    ...
    var varToUpdateIndex = upsertTemplate.vars.findIndex(
      (i) => i.QRef === modifyVarPopup.QRef,
    );
    ...
    var actualQuery = upsertTemplate.updateQuery.find(
      (i) => i.QRef === modifyVarPopup.QRef,
    );
    var newQuery;
    if (actualQuery === undefined) {
      ...
      actualQuery = upsertTemplate.insertQuery.find(
        (i) => i.QRef === modifyVarPopup.QRef,
      );
      if (actualQuery === undefined) {
        ...
        setUpsertTemplate(
          (prevState) => ({
            ...
            updateQuery: [
              ...
              {
                query: `UPDATE "Var" SET name='${data.name}', template=${varToUpdate.template}, type=${data.type}, um=${data.um}, logic_state=${data.logic_state}, comment='${data.comment}', fixed_id=${data.fixed_id} WHERE name = '${modifyVarPopup.name}' AND template = templateId;`,
                QRef: varToUpdate.QRef,
              },
            ],
            vars: vars,
          }),
        ...
        newQuery[newQuery.findIndex((i) => i.QRef === varToUpdate.QRef)] = {
          query: `INSERT INTO "Var" (id, name, template, type, um, logic_state, comment, fixed_id) VALUES (DEFAULT, '${varToUpdate.name}', ${varToUpdate.template}, ${varToUpdate.type}, ${varToUpdate.um}, ${varToUpdate.logic_state}, '${varToUpdate.comment}', ${varToUpdate.fixed_id});`,
          QRef: varToUpdate.QRef,
        };
        setUpsertTemplate(
          (prevState) => ({
      ...
      newQuery[newQuery.findIndex((i) => i.QRef === varToUpdate.QRef)] = {
        query: `UPDATE "Var" SET name='${varToUpdate.name}', template=${varToUpdate.template}, type=${varToUpdate.type}, um=${varToUpdate.um}, logic_state=${varToUpdate.logic_state}, comment='${varToUpdate.comment}', fixed_id=${varToUpdate.fixed_id} WHERE name = '${modifyVarPopup.name}' AND template = templateId;`,
        QRef: varToUpdate.QRef,
      };
      setUpsertTemplate(
        (prevState) => ({
  ...
  const deleteVar = () => {
    ...
    var deleteQuery =
      upsertTemplate.insertQuery.find((i) => i.QRef !== deletePopup.QRef) ===
      undefined
        ? [
            ...upsertTemplate.deleteQuery,
            {
              query: `DELETE FROM "Var" WHERE name = '${deletePopup.name}' AND template = templateId;`,
    ...
    setUpsertTemplate((prevState) => ({
      ...prevState,
      insertQuery: upsertTemplate.insertQuery.filter(
        (i) => i.QRef !== deletePopup.QRef,
      ),
      updateQuery: upsertTemplate.updateQuery.filter(
        (i) => i.QRef !== deletePopup.QRef,
      ),
              ...
              return (
                <TableRow key={item.id}>
                  <TableCell className={tableStyles.cell}>
                    {item.fixed_id !== undefined ? item.fixed_id : ''}
                  </TableCell>
