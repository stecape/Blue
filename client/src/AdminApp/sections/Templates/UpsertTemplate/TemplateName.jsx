...
function TemplateName(props) {
  ...
  const InlineValidation = (value) => {
    ...
    setUpsertTemplate((prevState) => ({
      ...prevState,
      name: value,
      templateNameNotValid:
        isPatternInvalid ||
        ctx.templates.find(
          (i) => i.name === value && i.id !== upsertTemplate.template,
        ) ||
        value === '',
    }));
  };
            ...
            onBlur={(e) => {
              if (
                ...
                upsertTemplate.create
                  ? setUpsertTemplate((prevState) => ({
                      ...prevState,
                      templateNameQuery: `INSERT INTO "Template" (id, name) VALUES (DEFAULT, '${upsertTemplate.name}') RETURNING id INTO templateId;`,
                    }))
                  : setUpsertTemplate((prevState) => ({
                      ...prevState,
                      templateNameQuery: `UPDATE "Template" SET name='${upsertTemplate.name}' WHERE id = ${upsertTemplate.template} RETURNING id INTO templateId;`,
                    }));
