...
export default function Home() {
  ...
  structs.types = ctx.types
    .filter((t) => !t.base_type)
    .map((type) => {
      let fields = ctx.fields
        .filter((field) => field.parent_type === type.id)
        .map((f) => {
          ...
          return {
            name: f.name,
            type:
              basetypes[ctx.types.find((t) => t.id === f.type).name] !==
              undefined
                ? basetypes[ctx.types.find((t) => t.id === f.type).name]
                : ctx.types.find((t) => t.id === f.type).name,
          };
        });
  ...
  structs.vars.forEach((v) => {
    ...
    let initTags = ctx.tags.filter(
      (t) =>
        ...
        (t.var === v.id &&
          device.id === t.device &&
          IsBaseType(v.id, ctx.vars, ctx.types).result),
    );
    initTags.forEach((t) => {
      let type =
        t.type_field !== null
      ...
      structs.tagType.push(
        ctx.types.find((t) => t.id === type).name.toUpperCase(),
      );
...
typedef struct {${structs.vars.map((v) => `\n\t${v.type} ${v.name};`).join('')}
} _HMI;
          ...
          <>
            <GridCell colSpan={6} className={gridStyles.item}>
              <TextContainer className={styles.textContainer}>
                <Button
                  ...
                  onClick={() => copyToClipboard(generate_HMI_h_Content())}
                >
                  <ContentCopySVGIcon />
                </Button>
            ...
            <GridCell colSpan={6} className={gridStyles.item}>
              <TextContainer className={styles.textContainer}>
                <Button
                  className={styles.floatingButton}
                  onClick={() => copyToClipboard(generate_HMI_c_Content())}
                >
                  <ContentCopySVGIcon />
                </Button>
