        ...
        <Table fullWidth className={tableStyles.table}>
          <TableHeader>
            <TableRow>
              ...
              <TableCell hAlign="left" style={{ minWidth: '300px' }} grow>
                Name
              </TableCell>
            ...
            {ctx.vars.length > 0 &&
              ...
              ctx.tags.map((item) => {
                ...
                if (item.type_field === null) {
                  ...
                  logic_stateItem = ctx.logicStates.find(
                    (i) => i.id === varItem.logic_state,
                  );
                } else {
                  var fieldItem = ctx.fields.find(
                    (i) => i.id === item.type_field,
                  );
                  //console.log(fieldItem, ctx, item)
                  typeItem = ctx.types.find((i) => i.id === fieldItem.type);
                  umItem = ctx.ums.find((i) => i.id === fieldItem.um);
                  logic_stateItem = ctx.logicStates.find(
                    (i) => i.id === fieldItem.logic_state,
                  );
                }
                console.log(item);
                return (
                  <TableRow key={item.id}>
                    <TableCell className={tableStyles.cell} hAlign="left">
                      {deviceItem !== undefined ? deviceItem.name : item.device}
                    </TableCell>
                    ...
                    <TableCell className={tableStyles.cell} hAlign="center">
                      {umItem !== undefined && umItem.name}
                    </TableCell>
                    <TableCell className={tableStyles.cell} hAlign="center">
                      {logic_stateItem !== undefined && logic_stateItem.name}
                    </TableCell>
                    <TableCell className={tableStyles.cell} hAlign="left">
                    ...
                    <TableCell className={tableStyles.cell} hAlign="center">
                      {item.value !== null && item.value.value.toString()}
                    </TableCell>
                    <TableCell />
                  </TableRow>
                );
