...
function UsersList () {
  ...
  const [deletePopup, setDeletePopup] = useState({ visible: false, id: 0, name: '', email: '' })
  const [modifyUserPopup, setModifyUserPopup] = useState({ visible: false, id: 0, name: '', email: '', role: '' })
  ...
  return(
    <>
            ...
            {ctx.users.map((item) => {
                return (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className={tableStyles.cell} hAlign="left">{item.email}</TableCell>
                    <TableCell className={tableStyles.cell} hAlign="left">{item.role}</TableCell>
                    <TableCell className={tableStyles.cell}>
                      <Button
                        buttonType="icon"
                        theme="error"
                        aria-label="Permanently Delete"
                        onClick={()=> setDeletePopup({visible: true, id: item.id, name: item.name, email: item.email})}
                      >
                        <DeleteFontIcon />
                      </Button>
                      <Button
                        buttonType="icon"
                        aria-label="Edit"
                        onClick={()=> setModifyUserPopup({visible: true, id: item.id, name: item.name, email: item.email, role: item.role})}
                      >
                        <EditFontIcon />
                      </Button>
                  </TableCell>
      ...
      <DeleteUserPopup 
        ...
        delUser={()=>{
          axios.post(`${serverIp}/api/removeUser`, {id: deletePopup.id})
      ...
      <UpsertUserPopup 
        ...
        modalType="full-page"
        upsertUser={(data)=>{
          axios.post(`${serverIp}/api/modifyUser`, {...data, id: modifyUserPopup.id})
            .then(setModifyUserPopup((prevState) => ({ ...prevState, visible: false })))
        }}
        cancelCommand={()=>{
