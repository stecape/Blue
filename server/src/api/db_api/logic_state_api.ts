...
import { ErrorResponse, AddLogicStateRequest, AddLogicStateResponse, ModifyLogicStateRequest, ModifyLogicStateResponse, RemoveLogicStateRequest, RemoveLogicStateResponse } from 'shared/types';
...
export default function (app: Application, pool: Pool) {
  ...
  app.post('/api/addLogicState', isAdmin, (req: Request<AddLogicStateRequest>, res: Response<AddLogicStateResponse | ErrorResponse>) => {
  ...
  app.post('/api/modifyLogicState', isAdmin, (req: Request<ModifyLogicStateRequest>, res: Response<ModifyLogicStateResponse | ErrorResponse>) => {
  ...
  app.post('/api/removeLogicState', isAdmin, (req: Request<RemoveLogicStateRequest>, res: Response<RemoveLogicStateResponse | ErrorResponse>) => {
    const queryString=`DELETE FROM "LogicState" WHERE id = ${req.body.id};`
    pool.query(queryString)
    .then(data => res.json({ result: data.rows, message: "Record correctly removed from LogicState " }))
    .catch(error => res.status(400).json({ code: error.code, detail: error.detail, message: error.detail }))
  })
