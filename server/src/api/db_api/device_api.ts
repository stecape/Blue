...
export default function (app: Application, pool: Pool) {
  ...
  app.post(
    '/api/addDevice',
    isAdmin,
    (req: Request<AddDeviceRequest>, res: Response<AddDeviceResponse | ErrorResponse>) => {
      const userIdValue =
        req.body.user_id !== undefined && req.body.user_id !== null && req.body.user_id !== 0
          ? req.body.user_id
          : 'NULL';
  ...
  app.post(
    '/api/modifyDevice',
    isAdmin,
    (req: Request<ModifyDeviceRequest>, res: Response<ModifyDeviceResponse | ErrorResponse>) => {
  ...
  app.get(
    ...
    (req: Request<GetDevicesRequest>, res: Response<GetDevicesResponse | ErrorResponse>) => {
      ...
      pool
        ...
        .then((data) => {
          res.json({ result: data.rows, message: 'Devices retrieved' });
        })
        .catch((error) =>
          res.status(400).json({ code: error.code, detail: error.detail, message: error.detail }),
        );
  ...
  app.post(
    '/api/removeDevice',
    isAdmin,
    (req: Request<RemoveDeviceRequest>, res: Response<RemoveDeviceResponse | ErrorResponse>) => {
