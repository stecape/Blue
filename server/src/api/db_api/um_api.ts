...
import { ErrorResponse, AddUmRequest, AddUmResponse, ModifyUmRequest, ModifyUmResponse, RemoveUmRequest, RemoveUmResponse } from 'shared/types';
...
export default function (app: Application, pool: Pool) {
  ...
  app.post('/api/addUm', isAdmin, (req: Request<AddUmRequest>, res: Response<AddUmResponse | ErrorResponse>) => {
  ...
  app.post('/api/modifyUm', isAdmin, (req: Request<ModifyUmRequest>, res: Response<ModifyUmResponse | ErrorResponse>) => {
  ...
  app.post('/api/removeUm', isAdmin, (req: Request<RemoveUmRequest>, res: Response<RemoveUmResponse | ErrorResponse>) => {
    const queryString=`DELETE FROM "um" WHERE id = ${req.body.id};`
    pool.query(queryString)
    .then(data => res.json({result: data.rows, message: `Record correctly removed from table "${req.body.table}"`}))
    .catch(error => res.status(400).json({code: error.code, detail: error.detail, message: error.detail}))
  })
