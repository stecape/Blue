...
export default function (app: Application, pool: Pool) {
  ...
  app.post(
    '/api/getVars',
    isAdmin,
    (req: Request<GetVarsRequest>, res: Response<GetVarsResponse | ErrorResponse>) => {
      ...
      pool
        ...
        .then((data) => {
          ...
          return pool.query(templateQuery).then((templateData) => {
            const templateName = templateData.rows[0]
              ? templateData.rows[0].name
              : 'Unknown Template';
