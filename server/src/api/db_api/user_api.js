...
export default function (app, pool) {
  ...
  app.get('/api/getUsers', isAdmin, (req, res) => {
  ...
  app.post('/api/modifyUser', isAdmin, (req, res) => {
    ...
    .then(data => {
      globalEventEmitter.emit('userUpdated', { userId: id })
  ...
  app.post('/api/removeUser', isAdmin, (req, res) => {
    ...
    .then(data => {
      globalEventEmitter.emit('userDeleted', { userId: req.body.id })
