...
const findOrCreateUser = async (profile, provider = 'google') => {
  ...
  try {
    ...
    if (userResult.rows.length > 0) {
      ...
      await dbPool.query(
        'UPDATE "User" SET name = $1, picture = $2 WHERE id = $3',
        [name, picture, userId]
      )
    } else {
      // Crea nuovo utente con ruolo 'user'
      const newUserResult = await dbPool.query(
        'INSERT INTO "User" (email, name, picture, role) VALUES ($1, $2, $3, $4) RETURNING *',
    ...
    if (identityResult.rows.length === 0) {
      ...
      await dbPool.query(
        'UPDATE "User_Identities" SET provider_user_id = $1 WHERE user_id = $2 AND provider = $3',
        [providerUserId, userId, provider]
      )
    }

    // 3. Recupera l'utente completo dal DB
    const finalUserResult = await dbPool.query(
