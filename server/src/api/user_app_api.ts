...
import { ErrorResponse, AuthenticatedRequest, GetDevicesRequest, GetDevicesResponse, GetUserDeviceDetailsRequest, GetUserDeviceDetailsResponse } from 'shared/types';
...
export default function (app: Application, pool: Pool) {  
  ...
  app.post('/api/user/devices', isAuthenticated, (req: Request<GetDevicesRequest>, res: Response<GetDevicesResponse | ErrorResponse> ) => {
    const { user } = req as unknown as AuthenticatedRequest;

    const queryString = `SELECT * FROM "Device" WHERE user_id = ${user.id}`;
    pool.query(queryString)
    .then(data => res.json({ result: data.rows, message: `${data.rowCount} device(s) returned` }))
    .catch(error => {
  ...
  app.post('/api/user/device', isAuthenticated, (req: Request<GetUserDeviceDetailsRequest>, res: Response<GetUserDeviceDetailsResponse | ErrorResponse>) => {
    const { user } = req as unknown as AuthenticatedRequest;
    ...
    .then(data => {
      if (data.rows.length === 0) {
        return res.status(400).json({ result: null, message: 'Device not found or access denied' })
      }
