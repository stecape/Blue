...
import {
  ...
  GetAllControlsRequest,
  GetAllControlsResponse,
  Control,
  Controls,
...
export default function (app: Application, pool: Pool) {
  app.post(
    ...
    isAuthenticated,
    async (
      req: Request<GetAllControlsRequest>,
      res: Response<GetAllControlsResponse | ErrorResponse>,
    ) => {
      try {
        ...
        devices.forEach((device) => {
          ...
          const deviceControls: Controls = templateVars.reduce((controls: Controls, variable) => {
            ...
            const varTags: DBTag[] = tags.filter(
              (tag) => tag.var === variable.id && tag.device === device.id,
            );
            ...
            varTags.forEach((tag) => {
              const control: Control = {
                ...
                fixed_id:
                  tag.fixed_id !== null && tag.fixed_id !== undefined ? Number(tag.fixed_id) : null,
                comment: tag.comment,
                fields: tags
