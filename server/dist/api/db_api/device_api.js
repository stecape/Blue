...
function default_1(app, pool) {
    ...
    app.post('/api/addDevice', auth_api_js_1.isAdmin, (req, res) => {
        ...
        pool.query({
            ...
            .then(data => {
            globalEventEmitter_js_1.default.emit('deviceAdded');
            globalEventEmitter_js_1.default.emit('refreshTags', { deviceId: req.body.id });
    ...
    app.post('/api/modifyDevice', auth_api_js_1.isAdmin, (req, res) => {
        ...
        pool.query({
            ...
            .then(data => {
            globalEventEmitter_js_1.default.emit('deviceUpdated');
            globalEventEmitter_js_1.default.emit('refreshTags', { deviceId: id });
            res.json({ result: data.rows[0], message: "Device updated" });
        })
            .catch(error => res.status(400).json({ code: error.code, detail: error.detail, message: error.detail }));
    });
    // Ottieni tutti i dispositivi (autenticato, filtrato per ruolo)
    app.get('/api/getDevices', auth_api_js_1.isAuthenticated, (req, res) => {
    ...
    app.post('/api/removeDevice', auth_api_js_1.isAdmin, (req, res) => {
        ...
        pool.query({
            ...
            .then(data => {
            globalEventEmitter_js_1.default.emit('deviceDeleted');
            globalEventEmitter_js_1.default.emit('DeleteTags', { deviceId: req.body.id });
