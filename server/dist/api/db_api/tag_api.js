...
function default_1(app, pool) {
          ...
          WHERE "Var".type IN (
            ...
            pool.query({
                ...
                const filteredVars = varsList.filter((v) => {
                    if (templateId)
                        return v[3] === templateId; // Filtra per template
                    if (typeId)
                        return dependentTypeIds.has(v[2]); // Filtra per dipendenze del tipo
    ...
    Err:    400
    */
    app.post('/api/deleteTags', auth_api_js_1.isAdmin, (req, res) => {
    ...
    Err:    400
    */
    app.post('/api/refreshTags', auth_api_js_1.isAdmin, (req, res) => {
    ...
    globalEventEmitter_js_1.default.on('refreshTags', (data) => {
